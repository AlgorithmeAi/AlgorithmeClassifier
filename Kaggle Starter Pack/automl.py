...
def automl(train, test, REF, restitution):
    ...
    al_model = AlgorithmeClassifier(n_layers=100)
    ...
    print("⏳ Entraînement Algorithme Classifier...")
    al_model.fit(X_train_encoded, y_train)
    al_model.get_lookalikes(X_backtest_encoded)
    ...
    al_snake_conf = al_model.predict_proba(X_backtest_encoded)[:, 1]
    # Résultats
    results = pd.DataFrame({
        ...
        "algorithme_snake_confidence" : al_snake_conf,
    })
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        exit()
    automl(sys.argv[1], sys.argv[2], sys.argv[3].split(","), "automl.csv")
